...
function setupEventListeners() {
    // Configura event listeners para todas as abas usando data-tab
...
function showTab(tabName) {
    // Remove classe active de todas as abas e sections
    document.querySelectorAll('.nav-tab').forEach(tab => tab.classList.remove('active'));
    document.querySelectorAll('.section').forEach(section => section.classList.remove('active'));
    
    // Adiciona classe active na aba e section corretas
...
// Dashboard: Top 5 de jogadores disponíveis
function renderDashboard() {
  const top5 = availablePlayers
    .slice()
    .sort((a, b) => a.Ranking - b.Ranking)
    .slice(0, 5);
  renderPlayerCards(top5, 'dashboardContainer');
}

// Top 40: Top 40 de jogadores disponíveis
function renderTop40Players() {
  const top40 = availablePlayers
    .slice()
    .sort((a, b) => a.Ranking - b.Ranking)
    .slice(0, 40);
...
function renderRankingTable() {
  const sortedAll = fullPlayersData
    .slice()
    .sort((a, b) => a.Ranking - b.Ranking);
  renderTable(sortedAll, 'rankingTable');
}
